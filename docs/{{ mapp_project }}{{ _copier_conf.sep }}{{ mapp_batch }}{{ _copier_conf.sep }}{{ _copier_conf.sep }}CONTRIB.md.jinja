...
ssh commons-server
tmux a -n sirius
cd git_repos/mapp-metabolomics-unit/{{recorded_repository_name or repository_name}}/docs/{{mapp_project}}/{{mapp_batch}}/results
```
...
paths:
  gnps_job_id: {{gnps_job_id}} # The GNPS job id you want to treat
  input_folder: ./met_annot_enhancer # The path were you want your GNPS job folder to be placed
  project_name: {{mapp_batch}} #ISDB_annot_LP_plantfungi_set # The name you want to give to your project, output resulst in data_out/project_name
  output_folder: ./met_annot_enhancer # the path for your output to be stored in
...
repond_params:
  ...
  var_one_header: '{{target_variable}}' # Optional (Run_line_x_line parameter)
...
```yaml
paths:
  docs: '/Users/pma/Dropbox/git_repos/mapp-metabolomics-unit/{{recorded_repository_name or repository_name}}/docs/{{mapp_project}}/{{mapp_batch}}/docs'
  output: '/Users/pma/Dropbox/git_repos/mapp-metabolomics-unit/{{recorded_repository_name or repository_name}}/docs/{{mapp_project}}/{{mapp_batch}}/results/stats' # Not mandatory, default is in the stats subdirectory
...
```yaml
mapp_project : {{mapp_project}}
mapp_batch : {{mapp_batch}}
met_annot_enhancer_folder : {{mapp_batch}}
gnps_job_id : {{gnps_job_id}}

dataset_experiment : 
  name: "{{mapp_batch}} LCMS metabolomics dataset"
  description: "{{mapp_batch_description}}"
...
filter_variable_metadata_one :
  mode : ''
  factor_name : '' # E.g. 'npc_pathway_canopus'
...
filter_variable_metadata_num :
  ...
  factor_name : '' # e.g 'sirius_confidencescore'
  level :  # Numerical value. E.g 0.5


target:
  sample_metadata_header: "{{target_variable}}" # This variable will be used throughout the whole script as you can see
...
```bash
met-annot-unifier-cli align-horizontally --canopus-file ./results/sirius/canopus_compound_summary.tsv --gnps-file ./results/met_annot_enhancer/{{gnps_job_id}}/nf_output/library/merged_results_with_gnps.tsv --gnps-mn-file ./results/met_annot_enhancer/{{gnps_job_id}}/nf_output/networking/clustersummary_with_network.tsv --sirius-file ./results/sirius/compound_identifications.tsv --isdb-file ./results/met_annot_enhancer/{{mapp_batch}}/{{mapp_batch}}_spectral_match_results_repond_flat.tsv --output ./results/tmp/{{mapp_batch}}_met_annot_unified.tsv
```